...
def get_cred(file_path="credentials.txt"):
    ...
    while True:
        username = input(f"   {PEACH}[>] Enter Login ID (Reg No, App No, or Email): {Fore.WHITE}").strip()
        if '@' not in username:
        ...
        print(f"   {Fore.WHITE}Login ID: {username}")
    ...
    while True:
        ...
        async with VtopClient(reg_no, password) as client:
           ...
           # --- SEMESTER AUTO-SELECTION (LOCAL CACHE) ---
            ...
            # Extract the TRUE Registration Number from the VTOP session memory
            true_reg_no = getattr(client._logged_in_student, "registration_number", reg_no) if hasattr(client, "_logged_in_student") and client._logged_in_student else reg_no
            display_id = true_reg_no if true_reg_no == reg_no else f"{true_reg_no} ({reg_no})"
            
            print(f"\n{Fore.CYAN}{'-'*65}\n")
            print(f" {Fore.WHITE}NAME:       : {Fore.GREEN}{student_name}")
            print(f" {Fore.WHITE}REG NO      : {Fore.CYAN}{display_id}")
            ...
            while True:
                ...
                if choice == '0':
                    ...
                    print(f"   {PEACH}=======================================")
                    new_user = input(f"   {PEACH}[>] Enter Login ID (Reg No, App No, or Email): {Fore.WHITE}").strip()
                    if '@' not in new_user:
